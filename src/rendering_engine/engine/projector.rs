...
pub fn project(
    camera_planes: &HashMap<PlaneDirection, Plane>,
    focal_length: f32,
    triangle: &Triangle3D,
) -> Triangle2D {
    let vertices: Vec<crate::rendering_engine::scene::model_2d::point::Point> = triangle
        .vertices()
        .iter()
        .map(|vertex: &Point3D| {
            let x_distance: f32 =
                vertex.distance_from_plane(&camera_planes.get(&PlaneDirection::YZ).unwrap());
            let y_distance: f32 =
                vertex.distance_from_plane(&camera_planes.get(&PlaneDirection::XZ).unwrap());
            let z_distance: f32 =
                vertex.distance_from_plane(&camera_planes.get(&PlaneDirection::XY).unwrap());

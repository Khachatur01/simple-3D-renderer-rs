...
use crate::rendering_engine::engine::model::z_buffered_triangle::{
    ZBufferedTriangle, ZBufferedVertex, ZBufferedVertices,
};
...
pub fn rasterize(z_buffered_triangle: &ZBufferedTriangle) -> ZBuffer {
    ...
    let mut pixel_buffer: DepthPixelBuffer =
        vec![vec![DepthPixel::default(); buffer_width + 1]; buffer_height + 1];

    let relative_vertices: ZBufferedVertices =
        z_buffered_triangle
            .vertices
            .map(|z_buffered_vertex: ZBufferedVertex| ZBufferedVertex {
    ...
    for i in 0..3 {
        let current: usize = i;
        let next: usize = if i == 2 { 0 } else { i + 1 };
...
fn get_fill_lines(pixel_buffer: &mut DepthPixelBuffer) -> Vec<(ZBufferedVertex, ZBufferedVertex)> {
    pixel_buffer
        .iter()
        .enumerate()
        .map(|(row, row_pixels)| get_fill_line(row, row_pixels))
        .collect()
}
...
fn fill(
    ...
    fill_lines
        .iter()
        .for_each(|fill_line: &(ZBufferedVertex, ZBufferedVertex)| {
...
fn create_bounding_box(z_buffered_triangle: &ZBufferedTriangle) -> (f32, f32, f32, f32) {
    let bounding_box: BoundingBox = z_buffered_triangle.vertices.iter().fold(
        (f32::MAX, f32::MAX, f32::MIN, f32::MIN),
        |bounding_box: BoundingBox, vertex: &ZBufferedVertex| {
            (
